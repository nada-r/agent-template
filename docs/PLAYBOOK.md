...
- Banning words one at a time ("no em dashes", "stop saying delve") doesn't scale — every doc still ships in the same AI voice. Give the agent a writing **system** instead: Orwell's six rules ("Politics and the English Language", 1946), in AGENTS.md.
...
- **Supply-chain cooldown**: never install a package version published <48h ago (AGENTS.md rule); Dependabot enforces 7-day / 14-day-major cooldowns mechanically, security patches exempt. npm worms spread within hours of a compromised publish — waiting out the first wave costs nothing.
